...
pub fn lua_user_data_method(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    for item in &impl_block.items {
        ...
        let arg_impls = params
            ...
            .map(|(i, typed)| {
                ...
                quote! {
                    let #mutability #arg_ident = <#ty as crate::engine::scripting::ConversionByValue>::perform_conversion_from_lua(#param_ident, lua)?;
                }
            }).collect::<Vec<_>>();
        ...
        let return_impl = if no_except {
            quote! {
                crate::engine::scripting::ConversionByValueReadOnly::perform_convertion_to_lua(&return_value, lua)
            }
        } else {
            quote! {
                crate::engine::scripting::ConversionByValueReadOnly::perform_convertion_to_lua(&return_value?, lua)
            ...
            MethodType::Take => {
                method_impls.push(quote! {
                    _methods.add_function(#lua_ident, |lua, (this, #param_idents): (mlua::AnyUserData, #param_types)| {
                        #(#arg_impls)*
                        let return_value = <Self as crate::engine::scripting::UserDataIntoSelf>::into_self(this)?.#ident(#arg_forward_impls);
                        #return_impl
                    });
                });
            }
    ...
    let methods_impl = quote! {
        impl #impl_generics crate::engine::scripting::UserDataMethodProvider for <#ty_name #ty_generics as crate::engine::scripting::LuaType>::LuaType #where_clause {
            fn add_methods<'lua, M: mlua::UserDataMethods<'lua, Self>>(_methods: &mut M) {
                #(#method_impls)*
                #impl_to_string
                <Self as crate::engine::scripting::UserDataOpsProvider>::add_ops(_methods);
            }
        }
    };
    let ops_impl = if impl_block.attrs.iter().find(|attr| attr.path().is_ident("ops_extra")).is_some() {
        ...
        quote! {
            impl #impl_generics crate::engine::scripting::UserDataOpsProvider for <#ty_name #ty_generics as crate::engine::scripting::LuaType>::LuaType #where_clause {}
        }
    };
